...
App::uses('OAuthUtility', 'OAuth.OAuthUtility');
	...
	protected function _clients() {
		...
		foreach ($clients as $client) {
			...
			if (Configure::read('OAuth.encrypt')) {
				$secret = OAuthUtility::decrypt($secret);
			}
			$this->out(sprintf('%-15s: %s', 'Client Id', $client['Client']['client_id']));
			$this->out(sprintf('%-15s: %s', 'Client Name', $client['Client']['name']));
			if ($this->params['secret']) {
				$this->out(sprintf('%-15s: %s', 'Client Secret', $secret));
			}
			$this->out(sprintf('%-15s: %s', 'Redirect URI', $client['Client']['redirect_uri']));
			$this->out("");
		}
